...
# Experiment 10 :- Setting Up a Simple Intrusion Detection System Using Snort
Objective
To configure and deploy Snort as a Network Intrusion Detection System (NIDS) to monitor network traffic and detect potential malicious activities.

Required Materials
Virtual Machine or physical machine with Linux (Ubuntu 20.04/22.04 recommended)
...
Method A: Installation from Repository (Recommended for Beginners)

bash
sudo apt install -y snort
Method B: Manual Installation from Source

bash
# Download latest Snort version
...
Set HOME_NET to your network (e.g., 192.168.1.0/24)

Set EXTERNAL_NET to any or !$HOME_NET

Configure paths for rules and logs
...
Identify your monitoring interface:
...
Register for Snort rules (free registration required):

bash
# Visit https://www.snort.org/ to register and obtain Oinkcode
...
bash
sudo nano /etc/snort/snort.conf
Uncomment/include the rule files you want to use
...
Run Snort in packet sniffing mode (to verify interface works):

bash
sudo snort -i [interface] -v
Step 7: Run Snort in IDS Mode
Execute as a daemon in NIDS mode:

bash
sudo snort -q -u snort -g snort -c /etc/snort/snort.conf -i [interface] -D
Alternative: Run in console mode (for testing):

bash
sudo snort -A console -q -u snort -g snort -c /etc/snort/snort.conf -i [interface]
Step 8: Generate Test Traffic and Monitor Alerts
Generate test attack traffic from another machine:

bash
# Using nmap for port scanning
nmap -sS [target_ip]

# Using hping3 for SYN flood test
sudo hping3 -S -p 80 --flood [target_ip]
Check Snort alerts:

bash
# View alerts in real-time (if running in console mode)
# Or check log files:
...
# Alert on HTTP directory traversal attempts
alert tcp any any -> $HOME_NET 80 (msg:"HTTP Directory Traversal Attempt"; content:"../"; nocase; sid:1000002;)
Step 10: Schedule Automatic Rule Updates
